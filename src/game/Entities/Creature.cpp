...
VendorItem const* VendorItemData::FindItemCostPair(uint32 item_id, uint32 extendedCost) const
{
    for (auto m_item : m_items)
    {
        ...
        if (m_item->item == item_id && m_item->ExtendedCost == extendedCost)
            return m_item;
    }
    return nullptr;
}
...
bool Creature::CanInteractWithBattleMaster(Player* pPlayer, bool msg) const
{
    ...
    if (!pPlayer->GetBGAccessByLevel(bgTypeId))
    {
        ...
        switch (bgTypeId)
        {
            ...
            case BATTLEGROUND_RV: pPlayer->GetPlayerMenu()->SendGossipMenu(10024, GetObjectGuid()); break;
            default: break;
        }
        return false;
    }
...
bool Creature::LoadCreatureAddon(bool reload)
{
    ...
    if (cainfo->pvp_flags != 0)
        SetByteValue(UNIT_FIELD_BYTES_2, UNIT_BYTES_2_OFFSET_PVP_FLAG, cainfo->pvp_flags);
    ...
    if (cainfo->auras)
    {
        for (uint32 const* cAura = cainfo->auras; *cAura; ++cAura)
        {
            ...
            SpellEntry const* spellInfo = sSpellTemplate.LookupEntry<SpellEntry>(*cAura);  // Already checked on load
...
bool Creature::hasWeapon(WeaponAttackType type) const
{
    ...
    return ((item && item->Class == ITEM_CLASS_WEAPON) || (type == OFF_ATTACK && CanDualWield()));
}
